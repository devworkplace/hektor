...
function inicializarCampoData(inputData, formCadastro) {
    inputData.mask(VALOR_MASCARA_DATA, { reverse: true });
    ...
    inputData.change(function () {
        if (!isNullOrEmpty($(this).val())) {
            formCadastro.data('bootstrapValidator').updateStatus(inputData.attr('name'), 'VALID');
        } else {
            formCadastro.data('bootstrapValidator').updateStatus(inputData.attr('name'), 'NOT_VALIDATED');
        }
    });
}
...
function renderColunaOpcoes(data, type, full) {
    ...
    if (metodoSelectLinha) {
        var cbxSelectLinha = $('<input id="cbxSelectLinha' + obterIdParaCheckBox(full) + '" type="checkbox" style="margin-right: 10px; cursor: pointer;" data-toggle="tooltip" data-original-title="' + textoPadraoMarcarLinhaSelect + '" onclick="atualizarLinhaListaLinhasSelect(this, ' + obterIdParaCheckBoxDataTable(full) + ')" />');
        ...
        .DataTable({
            ...
            fnServerData: function (sSource, aoData, fnCallback) {
                $.ajax({
                    ...
                    url: metodoGet,
                    data: JSON.stringify({ requestModel: JSON.stringify(aoData) }),
...
function salvarRegistro(auxFormCadastro, auxDvFormCadastro, auxDvTabelaPrincipal, auxUrl, auxData, tituloPagina, continuar, limparParaContinuar, passarDireto) {
    auxFormCadastro.data('bootstrapValidator').validate();
    if (auxFormCadastro.data('bootstrapValidator').isValid() || passarDireto) {
        mostrarPopup();
